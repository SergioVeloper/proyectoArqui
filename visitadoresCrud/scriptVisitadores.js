...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const elemento = document.getElementById("visitadores");

    // Agrega un listener para el evento 'click'
    elemento.addEventListener("click", function () {
        cargarVisit("visitadoresCrud/listar.php");
    });
});

function insertarVisit(){
    var contenedor = document.getElementById('contenido');
    var ajax = new XMLHttpRequest();
    ...
    ajax.open("GET", "visitadoresCrud/formulario_insertar.php", true);
    ajax.send();
}

function insertarVisitador() {
    var contenedor = document.getElementById('contenido');
    var ajax = new XMLHttpRequest();
    var formulario = document.getElementById('formInsertarVisitadores');
    ...
    ajax.onreadystatechange = function () {
        if (ajax.readyState == 4) {
            ...
            if (ajax.responseText.trim() === "success") {
                // Si la respuesta es "success", llamar a la función listarVisit()
                listarVisit();
            } else {
    ...
    ajax.open("POST", "visitadoresCrud/insertar.php", true);
    ajax.send(enviarForm);
}


function listarVisit(parametros){
    var elemento = document.getElementById('contenido');
    var ajax = new XMLHttpRequest();
    if(parametros){
        ajax.open("get", "visitadoresCrud/listar.php?"+parametros, true);
    }
    else{
        ajax.open("get", "visitadoresCrud/listar.php", true);
    }
    ajax.onreadystatechange = function(){
        if(ajax.readyState == 4){
...
function editarVisitador(id){
    ...
    ajax.open("GET", "visitadoresCrud/formulario_editar.php?id="+id, true);
    ajax.send()
}
...
function editarV() {
    var contenedor = document.getElementById('contenido');
    var ajax = new XMLHttpRequest();
    formulario = document.getElementById('formEditVisit');
    ...
    ajax.onreadystatechange = function () {
        if (ajax.readyState == 4) {
            ...
            listarVisit();  // Aquí llamas a listar() para cargar la lista de muestras médicas
        }
    }
    
    ajax.open("POST", "visitadoresCrud/editar.php", true);
    ajax.send(datosForm);
}


function eliminarVisitador(id){
    var contenedor = document.getElementById('contenido');
    var ajax = new XMLHttpRequest();
    idRescatado = "id="+id;
    ajax.onreadystatechange = function (){
    ...
    ajax.open("GET", "visitadoresCrud/eliminar.php?"+idRescatado, true);
    ajax.send();
}
...
function mostrarEspecialidad(id){
    ...
    if(id){
        ajax.open("GET", "visitadoresCrud/especialidad.php?especialidad_id="+id, true);
    }
    else{
        ajax.open("GET", "visitadoresCrud/especialidad.php", true);
    }
...
function mostrarSubEspecialidad(id){
    ...
    if(id){
        ajax.open("GET", "visitadoresCrud/subespecialidad.php?idMM="+id, true);
    }
    else{
        var id =  document.getElementById('seleccionarEsp').value;
        ajax.open("GET", "visitadoresCrud/subespecialidad.php?id="+id, true);
    }
